...
int otaNO = 7;//ลำดับของบอร์ดที่จะทำการอัพโหลด([แปลงที่ 1 ใช้เลข 1])
LiquidCrystal_I2C lcd(0x27, 20, 4);
const char *topic_soil1 = "soil13";
const char *topic_soil2 = "soil14";
...
int Rsoil2=36; //ขา pin ในการอ่านค่าเซ็นเซอร์ความชื้นในดินขาที่ 2
float soil1,soil2; //ตัวแปรในการเก็บค่าคาริเบทเซนเซอร์
float Csoil1,Csoil2,SumSoil1,SumSoil2;
...
//ฟังชั่นในการเปลี่ยนค่าความชื้นในดินที่อ่านเป็นค่า อนาล็อกเป็นเปอร์เว็น(วิธีการนี้ได้มาจากการทดลองและสมการจาก Ai)
...
void mq(){
  ...
  client.publish(("BV"+String(otaNO)).c_str(),String(vBat).c_str());//ส่งค่าแรงดันของแบตเตอร์รี่ไปยัง node red
  client.publish(("BP"+String(otaNO)).c_str(),String(percent).c_str());//ส่งค่าเปอร์เซ็นของแบตเตอร์รี่ไปยัง node red
  client.publish(("soil"+String(TGS-1)).c_str(),String(soil1).c_str());//ส่งค่าความชื้นในดินที่ 15 cm
  client.publish(("soil"+String(TGS)).c_str(),String(soil2).c_str());//ส่งค่าความชื้นในดินที่ 30 cm
}
...
void reconnect() {
  if (!client.connected()) {//เงื่อนไขเมื่อเชื่อมต่อกับ mqtt ไม่ได้
    ...
    if (client.connect("npru_ee_soil_moisture_7")) {//ใส่ข้อความอะไรก็ได้ที่ท่านคิดว่าจะไม่ซ้ำกับผู้อื่นหรือไม่ซ้ำกันในจำนวนมาก(หากไม่สามารถเชื่อมต่อกับ mqtt ได้ลองทำการเปลี่ยนข้อความ)
...
void loop() {
  if (otaUp==0){//เงื่อนไขในเพื่อใช้ในการอัพโหลดโปรแกรมแบบไร้สาย(0=ทำงานปกติและไม่พร้อมอัพโหลดโปรแกรมแบบไร้สาย) 
    ...
    lcd.print("in 10 seconds");//ทำการแสดงข้อความ
      for (int i=1;i<=10;i++){//ทำการวนลูปเพื่อเก็บค่าเซ็นเซอร์ 10 ครั้งใน 10 วินาที
        ...
        int combined_soil_moisture1=analogRead(Rsoil1);//ทำการอ่านค่าอนาล็อกของเซ็นเซอร์วัดความชื้นในดินที่ 15 cm
        int combined_soil_moisture2=analogRead(Rsoil2);//ทำการอ่านค่าอนาล็อกของเซ็นเซอร์วัดความชื้นในดินที่ 30 cm
        Csoil1 = combinedPredict(combined_soil_moisture1);//ทำการเปลี่ยนค่าอนาล็อกให้เป็นเปอร์เซ็นที่ 15 cm
        Csoil2 = combinedPredict(combined_soil_moisture2);//ทำการเปลี่ยนค่าอนาล็อกให้เป็นเปอร์เซ็นที่ 30 cm
        SumSoil1 = SumSoil1+Csoil1;//นำค่าความชื้นในดินในแต่ละวิของ 15 cm มารวมเพื่อนำไปหาค่าเฉลี่ย
        SumSoil2 = SumSoil2+Csoil2;//นำค่าความชื้นในดินในแต่ละวิของ 30 cm มารวมเพื่อนำไปหาค่าเฉลี่ย
        Serial.println("soil1="+String(Csoil1));//แสดงค่าความชื้นในดินที่ 15 cm บน Serial moniter
        Serial.println("soil2="+String(Csoil2));//แสดงค่าความชื้นในดินที่ 30 cm บน Serial moniter
  ...
  if (otaUp==0) {//เงื่อนไขในเพื่อใช้ในการอัพโหลดโปรแกรมแบบไร้สาย(0=ทำงานปกติและไม่พร้อมอัพโหลดโปรแกรมแบบไร้สาย)
  Serial.println("ota="+String(otaUp));
