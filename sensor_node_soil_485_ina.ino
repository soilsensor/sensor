...
const byte soil[] = {0x01 ,0x03 ,0x00 ,0x00 ,0x00 ,0x02 ,0xC4 ,0x0B};
byte values[20];
...
//#define WiFi_SSID "............"//ssid หรือชื่อของ WiFi ที่ต้องการทำการเชื่อมต่อ
//#define WiFi_PASSWORD "kkkkoooo" //password หรือรหัสในการเชื่อมต่อ WiFi //password หรือรหัสในการเชื่อมต่อ WiFi
...
float getBatteryPercent(float Vbb) {
  float aa = Vbb-11.6;
  float vper = (aa/1.8)*100;//เปลี่ยนค่าจาก 3.3 เป็นค่าเปอร์เซ็น
...
float readBatteryVoltage() {
  ...
  float Vbb = busvoltage + (shuntvoltage / 1000);
  Serial.println("Vbb = "+String(Vbb));
  return Vbb;
}
...
void mq(){
  ...
  percent = getBatteryPercent(vBat);//ดึงค่าเปอร์เซ็นแบตเตอร์รี่
  ...
  client.publish(topic_soil2,String(soil1).c_str());//ส่งค่าความชื้นในดินที่ 15 cm
}
...
void reconnect() {
  if (!client.connected()) {//เงื่อนไขเมื่อเชื่อมต่อกับ mqtt ไม่ได้
    ...
    if (client.connect("mqtt_mois_RS485_4")) {//ใส่ข้อความอะไรก็ได้ที่ท่านคิดว่าจะไม่ซ้ำกับผู้อื่นหรือไม่ซ้ำกันในจำนวนมาก(หากไม่สามารถเชื่อมต่อกับ mqtt ได้ลองทำการเปลี่ยนข้อความ)
...
void aggg(){
    HTTPClient http;//สร้างตัวแปรที่จะใช้ในการเขียนฟังชั่นในการส่งขึ้น google sheet
        ...
        String url = "https://script.google.com/macros/s/AKfycbx8epk8-NIs751vmMJWxTk1z1fH17LN6obBdw__1KjgpT0yMj9VZk_NLeyCMWBQCpAU/exec?data"+String(TGS)+"="+String(soil1);
...
void LLCd(){
  ...
  lcd.setCursor(1,1);//เริ่มแสดงข้อความที่ 1 บรรทัดที่ 1
  lcd.print("Soil = ");//แสดงข้อความขึ้นหน้าจอ LCD
...
void Get_soil() {
  ...
  { //ทำการหยุดส่งชุดข้อมูลโดยการให้ค่า DE และ RE เป็น LOW
    ...
    Ssoil1 = ((values[3]*256)+values[4])*0.1;
...
lcd.print("in 10 seconds");//ทำการแสดงข้อความ
  for (int i=1;i<=11;i++){//ทำการวนลูปเพื่อเก็บค่าเซ็นเซอร์ 10 ครั้งใน 10 วินาที
    reconnect();//ฟังชั่นในการตรวบสอบ mqtt
    client.loop();//เป็นฟังชั่นที่ช่วยให้การรับข้อมูลกลับมาจาก node red (การจะรับข้อมูลกลับมาได้จำเป็นต้องมีฟังชั่นดังกล่าวแต่หากไม่มีการรับข้อมูลกลับมาจาก node red ไม่จำเป็นต้องใช้ฟังชั่นดังกล่าว)
